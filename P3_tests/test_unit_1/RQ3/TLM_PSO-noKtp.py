            ...
            for group_id in range(num_path_sets):
                if group_id in all_group_results:
                    ...
                    if data_key in group_data and exp_id < len(group_data[data_key]):
                        ...
                        # 🔧 修改:如果没有数据,填入默认值,添加iterations和evaluations处理
                    ...
                    # 🔧 修改：如果组不存在，填入默认值，添加iterations处理
                    if file_type == "success":
                        ws.cell(row=exp_id + 2, column=group_id + 2, value=0)
                    elif file_type == "time":
                        ws.cell(row=exp_id + 2, column=group_id + 2, value=0)
...
class PSO:
    ...
    def __init__(self, mode, target_path, global_comm=None, high_partner_rank=None,
        ...
        self.evaluation_count = 0  # 🔧 新增:个体评价次数计数器
...
def run_single_experiment(experiment_id, comm, rank, size, num_path_sets, procs_per_group,
    ...
    if group_rank == 0:
        ...
        result = {
            ...
            'evaluations': total_evaluations,  # 🔧 新增:总评价次数
        ...
        if rank == 0:
            ...
            if experiment_id >= WARMUP_EXPERIMENTS:
                for group_id, result in experiment_results.items():
                    ...
                    # 🔧 步骤7:记录每次实验的详细结果用于Excel输出,包含迭代次数和评价次数
                    ...
                    group_stats[group_id]['evaluations_list'].append(result['evaluations'])  # 🔧 新增:评价次数
