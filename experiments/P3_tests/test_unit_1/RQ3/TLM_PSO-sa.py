...
# 🔧 修改：新增Excel输出功能 - 将所有路径组的结果合并保存到Excel文件，包含迭代次数和评价次数
...
class PSO:
    ...
    def __init__(self, mode, target_path, global_comm=None, high_partner_rank=None,
                 high_similarity=0.0, group_color=0, local_rank=0):
    ...
    def calculateFitnessBasedOnPath(self, position):
        self.evaluation_count += 1  # ← 新增：每次评价计数+1
...
class PSO5D:
    ...
    def __init__(self, targetPaths, comm, global_comm=None, high_partner_rank=None,
                 high_similarity=0.0, group_color=0):
...
# 🔧 步骤3：在run_single_experiment中收集评价次数
def run_single_experiment(experiment_id, comm, rank, size, num_path_sets, procs_per_group):
    ...
    if rank == 0:
        ...
        current_group_info = {
            ...
            'p1_sim': p1_pair_sim.get(0, 0.0),
            'p2_partner': p2_pairing.get(0, None),
            'p2_sim': p2_pair_sim.get(0, 0.0),
            'p3_partner': p3_pairing.get(0, None),
            'p3_sim': p3_pair_sim.get(0, 0.0),
            'global_partner': global_pairing.get(0, None),
            'global_sim': global_pair_sim.get(0, 0.0)
        }
    ...
    if group_rank == 0:
        ...
        if rank != 0:
            ...
            p1_sim = current_group_info['p1_sim']
            p2_partner = current_group_info['p2_partner']
            p2_sim = current_group_info['p2_sim']
            p3_partner = current_group_info['p3_partner']
            p3_sim = current_group_info['p3_sim']
            global_partner = current_group_info['global_partner']
            global_sim = current_group_info['global_sim']
        else:
            p1_partner = current_group_info['p1_partner']
            p1_sim = current_group_info['p1_sim']
            p2_partner = current_group_info['p2_partner']
        ...
        # 计算高相似度伙伴实际rank并发送给组内进程
        if p1_partner is not None:
            p1_partner_rank = p1_partner * procs_per_group + 1  # 计算伙伴进程1的全局rank
            group_comm.send((p1_partner_rank, p1_sim), dest=1, tag=666)
        else:
            group_comm.send((None, 0.0), dest=1, tag=666)

        if p2_partner is not None:
            p2_partner_rank = p2_partner * procs_per_group + 2  # 计算伙伴进程2的全局rank
            group_comm.send((p2_partner_rank, p2_sim), dest=2, tag=667)
        else:
            group_comm.send((None, 0.0), dest=2, tag=667)

        # 发送进程3的高相似度伙伴信息
        if p3_partner is not None:
            p3_partner_rank = p3_partner * procs_per_group + 3  # 计算伙伴进程3的全局rank
            group_comm.send((p3_partner_rank, p3_sim), dest=3, tag=670)
        else:
            group_comm.send((None, 0.0), dest=3, tag=670)
    ...
    if group_rank == 1:
        ...
        # 接收来自进程0的高相似度配对信息
        status = MPI.Status()
        partner_info = group_comm.recv(source=0, tag=666, status=status)
        partner_rank, partner_similarity = partner_info

        pso_local = PSO(
            mode="full",
            target_path=local_target_path,
            global_comm=comm,
            high_partner_rank=partner_rank,
            high_similarity=partner_similarity,
        ...
        # 接收来自进程0的高相似度配对信息
        status = MPI.Status()
        partner_info = group_comm.recv(source=0, tag=667, status=status)
        partner_rank, partner_similarity = partner_info
        ...
        # 接收来自进程0的高相似度配对信息
        status = MPI.Status()
        partner_info = group_comm.recv(source=0, tag=670, status=status)
        partner_rank, partner_similarity = partner_info
...
def main():
    ...
    # 🔧 步骤4：初始化统计数据结构，添加适应值列表和详细记录列表，包含迭代次数和评价次数
    if rank == 0:
        group_stats = {}
        for group_id in range(num_path_sets):
        ...
        if rank == 0:
            ...
            if experiment_id >= WARMUP_EXPERIMENTS:
                for group_id, result in experiment_results.items():
                    ...
                    # 🔧 步骤5：记录每次实验的详细结果用于Excel输出，包含迭代次数和评价次数
    ...
    if rank == 0:
        ...
        # 🔧 步骤5：保存详细结果到Excel文件，包含迭代次数和评价次数
        print(f"\n{'=' * 60}")
